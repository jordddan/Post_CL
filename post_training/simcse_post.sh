...
for alpha in ${ALPHA[@]};do
    for seed in ${SEED[@]};do
        ...
        log_path=/opt/data/private/qd/noise_master/post_training/log/post_training/${data}cl${alpha}
        rm -rf log_path
        mkdir -p $log_path
            CUDA_VISIBLE_DEVICES=$gpu_device python simcse_post.py \
                --bert_type $model --data ${data}cl${alpha} \
