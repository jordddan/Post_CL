...
def train(args, mymodel, optimizer, train_data, valid_data=None, test_data=None):
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        for i, data in enumerate(train_data):
            ...
            input_ids, input_ids_mask_aug, attention_mask, labels, _ = [Variable(elem.cuda()) for elem in data]
            ...
            # out_aug = F.log_softmax(logits_aug)
...
def evaluate(data, epoch, mymodel, mode):
    ...
    for j, batch in enumerate(data):   
        input_ids, input_ids_aug, attention_mask, labels, _ = [Variable(elem.cuda()) for elem in batch]
